...
class ImageCarousel extends React.Component {
	...
	render() {
		...
		for (const [i, url] of this.props.itemImages.entries()) {
			...
			images.push(
				React.createElement("div", {className: `carousel-item ${i == 0 ? "active" : ""}`}, 
					React.createElement("img", {src: `https://drive.google.com/uc?id=${fileID}&export=download`})
				)
			);

			indicators.push(
				React.createElement("li", {"data-target": carouselID, 
					"data-slide-to": `${i}`, 
		...
		return (
			React.createElement("div", {id: carouselID, className: "carousel slide", "data-ride": "carousel"}, 
				carouselIndicators, 
				carouselContents, 
				React.createElement("a", {className: "carousel-control-prev", href: `#${carouselID}`, "data-slide": "prev"}, 
					React.createElement("span", {className: "carousel-control-prev-icon"})
				), 
				React.createElement("a", {className: "carousel-control-next", href: `#${carouselID}`, "data-slide": "next"}, 
...
class ProductContainer extends React.Component {
	...
	render() {
		const regex = /file[/]d[/](.+)[/]/;
		const fileID = this.props.itemImages[0].match(regex)[1];
		const fileURL = `https://drive.google.com/uc?id=${fileID}&export=download`;
		const bullets = this.props.itemDescription.map(bullet => React.createElement("li", null, bullet));

		return (
			React.createElement("div", {className: "collapsible", style: this.props.style}, 
				React.createElement("input", {className: "collapsing toggle", type: "checkbox", 
						onClick: () => this.changeFocus()}), 
				React.createElement("div", {className: "collapsing island"}, 
					React.createElement("div", {className: "info maximized"}, this.props.itemName), 

					React.createElement("div", {className: "columns"}, 
						React.createElement(ImageCarousel, {itemImages: this.props.itemImages, itemName: this.props.itemName})
					), 
					React.createElement("div", {className: "columns"}, 
						React.createElement("ul", {className: "description-bullets info"}, ...bullets)
					), 

					React.createElement("div", {className: "image center", src: fileURL, style: {background: `url(${fileURL})`}}), 
					React.createElement("div", {className: "center info minimized name"}, this.props.itemName), 
					React.createElement("div", {className: "center info minimized price"}, this.props.itemPrice), 

					React.createElement("button", {id: `add-${this.props.itemName.toLowerCase().replaceAll(" ", "-")}`, 
							className: "purchasing info", 
							onClick: () => this.addToCart()}, 
						"Add to cart"
					), 

					React.createElement("div", {className: "purchasing"}, 
						React.createElement("div", {className: "info price per"}, `${this.props.itemPrice}`)
						/* <input type="number" className="info price multiplier" min="1" defaultValue="1"></input> */
					)
				), 
				React.createElement("span", {className: "carousel-control-prev-icon mock-check"})
			)
		);
	}
}
...
class Inventory extends React.Component {
	...
	render() {
		...
		for (const [i, product] of this.props.products.entries()) {
			...
			const item = React.createElement(ProductContainer, {
...
(function () {
	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			var inventory = React.createElement(Inventory, {products: jsonData["Products"]});
			cartCards = jsonData["Products"].map(product => renderCartCard(product));

			ReactDOM.render(inventory, document.getElementById("shopping"));
		}
	}
